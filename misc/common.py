...
def ecdc_decode(ecdc, out=None):
	fmt = out.rsplit(".", 1)[-1] if out else "webm"
	ts = ts_us()
...
find_urls = lambda url: url and regexp(r"""(?:http|hxxp|ftp|fxp)s?:\/\/[^\s`|"'\])>]+""").findall(url)
is_url = lambda url: url and isinstance(url, (str, bytes)) and regexp(r"""^(?:http|hxxp|ftp|fxp)s?:\/\/[^\s`|"'\])>]+$""").fullmatch(url)
is_discord_url = lambda url: url and regexp(r"^https?:\/\/(?:\w{3,8}\.)?discord(?:app)?\.(?:com|net)\/").findall(url) + regexp(r"https:\/\/images-ext-[0-9]+\.discordapp\.net\/external\/").findall(url)
is_discord_attachment = lambda url: url and regexp(r"^https?:\/\/(?:\w{3,8}\.)?discord(?:app)?\.(?:com|net)\/attachments\/").search(str(url))
is_tenor_url = lambda url: url and regexp(r"^https?:\/\/tenor.com(?:\/view)?/[\w\-]+-[0-9]+").findall(url)
is_imgur_url = lambda url: url and regexp(r"^https?:\/\/(?:\w\.)?imgur.com/[\w\-]+").findall(url)
is_giphy_url = lambda url: url and regexp(r"^https?:\/\/giphy.com/gifs/[\w\-]+").findall(url)
is_youtube_url = lambda url: url and regexp(r"""^https?:\/\/(?:\w{1,5}\.)?youtu(?:\.be|be\.com)\/[^\s<>`|"']+""").findall(url)
is_youtube_stream = lambda url: url and regexp(r"^https?:\/\/r+[0-9]+---.{2}-[\w\-]{4,}\.googlevideo\.com").findall(url)
is_deviantart_url = lambda url: url and regexp(r"""^https?:\/\/(?:www\.)?deviantart\.com\/[^\\s<>`|"']+""").findall(url)
is_reddit_url = lambda url: url and regexp(r"^https?:\/\/(?:\w{2,3}\.)?reddit.com\/r\/[^/\W]+\/").findall(url)
is_redgifs_url = lambda url: url and regexp(r"^https?:\/\/(?:\w{2,3}\.)?redgifs.com\/\w{2,6}\/[^/\W]+").findall(url)
